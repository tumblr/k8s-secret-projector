PACKAGE  = github.com/tumblr/k8s-secret-projector
BINARY   = k8s-secret-projector
...
DOCKER_IMAGE_BASE = tumblr/k8s-secret-projector
DOCKER_TAG = $(VERSION)
...
.PHONY: all
all: fmt lint vendor plugins | ; $(info $(M) building executable…) @ ## Build program binary
	$Q $(GO) build \
		-tags release \
...
$(TEST_TARGETS): test
check test tests: fmt lint vendor plugins | ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests
...
.PHONY: plugins
plugins: vendor | ; $(info $(M) retrieving plugins…)
	$Q for plugin in ./plugins/encryption/* ; do \
		  $(GO) build -buildmode=plugin $$plugin && \
			echo "Compiled plugin $$(basename $$plugin).so" ; \
		done
